...
class ImmacAgent(nn.Module):
    ...
    def generate_message_2(self, x, hidden):
        ...
        comm_rate = torch.where(imp > self.args.threshold, torch.ones_like(imp),
                                torch.zeros_like(imp))
        comm_rate = comm_rate.mean(dim=-1, keepdim=True)  # [batch_size, 1]

        # imp = - flaot('inf') when agent don't send message
        imp = torch.where(imp > self.args.threshold, imp,
                          torch.full_like(imp, -9999999))  # [batch_size, n_agents]
